...
#Sebastian didn't clap for my 12th anniversary.  
#Note to self: Fail him.  
#Note to self: Sebastian corrected me......hmm
...
"A GREAT example of an outer join is joining batters and pitcher tables!" - Greg
...
#Question 1: 
Batting %>% filter(playerID == "ramirma02") %>%  group_by(playerID, yearID) %>% summarise(HR = sum(HR)) %>% filter(HR > 30) %>% left_join(People %>% select(playerID,nameFirst, nameLast), by = c("playerID"="playerID")) %>% arrange(-HR)

#Question 2: 
Batting %>% filter(playerID == "ramirma02") %>% group_by(playerID, yearID) %>% summarise(HR = sum(HR)) %>% left_join(People %>% select(playerID,birthYear,nameFirst, nameLast), by = c("playerID"="playerID")) %>% mutate(age = yearID - birthYear) %>% arrange(-HR)

#Question 3
#Manny homeruns
mannyHR <- Batting %>% filter(playerID == "ramirma02") %>% group_by(playerID, yearID) %>% summarise(mannyHR = sum(HR))

#Total homeruns
totalHR <- Batting  %>% group_by(yearID) %>% summarise(totalHR = sum(HR))

mannyHR %>% inner_join(totalHR, by = c("yearID" = "yearID")) %>% mutate(fracHR = mannyHR/totalHR) %>% arrange(-fracHR)
...
Use the Batting, Pitching, and People tables in the Lahman package to answer the following questions.
 ...
 - Identify the name and year of every player who has hit at least 50 home runs in a single season. Which player had the lowest batting average in that season? (Note: Batting average is H/AB)
